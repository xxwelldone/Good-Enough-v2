...
async function callResults(event) {
...
function buildResults(result) {
    ...
    for (let i = 0; i < result.results.length; i++) {
        ...
        resultImg.setAttribute("src", `https://image.tmdb.org/t/p/w500${result.results[i].poster_path}`);
        ...
        resultRating.innerText = `${calcPercent(result.results[i].vote_average)}% Good Enough`;
        ...
        resultDetails.append(resultTitle, resultRating, resultLikeIcon, resultOverview);
        resultContent.append(resultImg, resultDetails);
        resultArticle === null || resultArticle === void 0 ? void 0 : resultArticle.append(resultContent);
    }
}
